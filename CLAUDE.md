...
| `GAMEDAY.md` · `playoffs-live.html` | **The playoffs are run LIVE from the field (Aug 21–22, 2026) — read `GAMEDAY.md` first whenever Curtis sends a result.** He ssh's in from the park and sends winners only (never runs; no ties on playoff day); each result is one cycle — append to `0821-playoff-results.csv` → `emit.sh` → `splice.py` → `live-test.js` → commit + push when his message authorizes it. `playoffs-live.html` (built 2026-08-20, UNTRACKED, staged at repo root) is the gameday cut of the bracket page: the field table and the entry desk are gone, the bracket draws the REAL results, the leaderboard is scored, and opening an entry lays it over the bracket card by card (✓ called it · ✗ had X · picks X). **Owner's instruction: it is NOT served at the `playoffs.html` route until he says entries are closed (≥ 15:00 Friday) — then `cp playoffs-live.html playoffs.html` per GAMEDAY.md §3.** Its island is the same `machine-data` with a `live: {updated, played}` object, which `emit_playoff_seeds` adds — and scores every entry, even 0 of 0 — whenever `--playoff-results` is given; without that flag the island is byte-identical to the prediction era's. Tooling (emit/splice/fix-result/test/screenshot scripts, jsdom + playwright) lives outside the repo in `~/.claude/projects/-Users-cube-Developer-cp-softball/gameday-tools/` |
